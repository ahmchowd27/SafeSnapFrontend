...
import { UpdateIncidentRequest } from '@/types';
...
const LocationPicker: React.FC<LocationPickerProps> = ({ onLocationSelect, selectedPosition }) => {
  ...
  return (
    <MapContainer 
      center={selectedPosition || [37.7749, -122.4194]} 
      zoom={13} 
...
const EditIncidentPage: React.FC = () => {
  ...
  const updateIncidentMutation = useMutation({
    mutationFn: (data: UpdateIncidentRequest) => incidentsApi.updateIncident(id!, data),
    onSuccess: () => {
      navigate(`/worker/incidents/${id}`);
    },
    onError: (error: Error) => {
  ...
  useEffect(() => {
    if (incident) {
      ...
      if (incident.imageUrls) {
        setUploadedImages(incident.imageUrls);
      }

      if (incident.audioUrls) {
        setUploadedAudio(incident.audioUrls);
      }
    }
  }, [incident, form]);
  ...
  const onSubmit = (data: IncidentFormData) => {
    ...
    updateIncidentMutation.mutate(incidentData);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg">Loading incident...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">The incident you're trying to edit doesn't exist.</p>
          <Link to="/worker">
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            <Link to={`/worker/incidents/${id}`}>
              <Button variant="ghost" size="sm">
                <ArrowLeft className="h-4 w-4 mr-2" />
                Back to Incident
              </Button>
            </Link>
            <h1 className="text-2xl font-bold text-gray-900 ml-6">Edit Incident</h1>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end space-x-4">
            ...
            <Button 
              type="submit" 
              disabled={updateIncidentMutation.isPending || isUploading}
            >
